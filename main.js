...
function notbefirst0rbelast_dot() {
    ...
    if (email[0] == '.' || email[email.length - 1] == '.') {
        alert("נקודה לא יכולה להיות האות הראשונה או האחרונה")
    }
...
function exact1_dot() {
    ...
    for (let character of email)
        if (character == '.')
            counter += 1
    if (counter == 0)
        alert("מייל לא יכול לכלול יותר מנקודה אחד")
...
function exact1shtrusdel() {
    ...
    for (let character of email)
        if (character == '@')
            counter += 1
    if (counter != 1)
        alert("מייל לא יכול לכלול יותר משטודל אחד")
...
function checkmaillsize() {
    ...
    if (email.length < 6) {
        alert("מייל חייב להיות לפחות 6 תווים")
    }
}
